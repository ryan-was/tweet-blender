...
function TB_start() {
    ...
    jQuery.each(jQuery('form.tb-widget-configuration'),function(i,obj){
        ...
        if (needWidgetHTML) {
            // add widget HTML
            widgetHTML = '<div id="' + widgetId + '-mc" class="conversation-box"><div class="tb_header">' +
                '<div class="tb_tools" style="background-image:url(' + TB_pluginPath + '/img/bg_sm.png)">' +
                '<a class="tb_refreshlink" href="javascript:TB_blend(\'' + widgetId + '\');" title="' + TB_labels.refresh + '"><img src="' + TB_pluginPath + '/img/ajax-refresh-icon.gif" alt="' + TB_labels.refresh + '" /></a>' +
                '</div></div>';
            
            if (TB_config.general_seo_tweets_googleoff && !isChart) {
                widgetHTML += '<!--googleoff: index--><ul class="tb_tweetlist list-conversation"></ul><!--googleon: index-->';
            }
            else if (isChart){
                widgetHTML += '<div id="' + widgetId + '-chart" class="tb_tweetchart"></div>';
            }
            else {
                widgetHTML += '<ul class="tb_tweetlist list-conversation"></ul>';
        ...
        for (widgetId in TB_config.widgets) {
            ...
            if (typeof(TB_config.widgets[widgetId].sources) == 'undefined' || TB_config.widgets[widgetId].sources == '') {
                ...
                if (typeof(TB_config.widgets[widgetId].chartType) == 'undefined') {
                    ...
                    if (jQuery('#'+widgetId + '-mc > ul.tb_tweetlist > li.tb_tweet').size() > 0) {
                        if (TB_tmp = jQuery('#'+widgetId + '-mc > ul.tb_tweetlist > div:last').attr('id')) {
                            TB_config.widgets[widgetId]['minTweetId'] = TB_tmp;
                        }
                        if (TB_tmp = jQuery('#'+widgetId + '-mc > ul.tb_tweetlist > div:first').attr('id')) {
                            TB_config.widgets[widgetId]['maxTweetId'] = TB_tmp;
                        }
                    }
                    TB_config.widgets[widgetId]['tweetsShown'] = jQuery('#'+widgetId + '-mc > ul.tb_tweetlist').children('div').size();
                    
                    // wire mouse overs to existing tweets
                    jQuery.each(jQuery('#' + widgetId + '-mc > ul.tb_tweetlist').children('div'),function(i,obj){ TB_wireMouseOver(obj.id); });
    
                        // wire target="_blank" on links
                        jQuery('.tweet a, .tweet-tools a, .tb_infolink').click(function(){
                            this.target = "_blank";
                        });
                }
...
function TB_initInfoBox(widgetId) {
    ...
    jQuery.each(TB_config.widgets[widgetId].sources.split(','),function(i,src) {
        ...
        if (TB_config.widgets[widgetId].sourcesCount > 0)
            TB_config.widgets[widgetId].sourcesHTML += ', '
...
function TB_checkComplete(widgetId,error) {
    if(typeof error !== 'undefined') {
        TB_hideLoader(widgetId);
        if (error == 'no_tweets_err')
            TB_showMessage(widgetId, 'notweets', TB_labels.no_tweets_err.format(TB_config.widgets[widgetId].sourcesHTML), true);
        else if (error === 'connection_err')
            TB_showMessage(widgetId, 'notweets', TB_labels.connection_err.format(TB_config.widgets[widgetId].sourcesHTML), true);
        else
            TB_hideMessage(widgetId,'notweets');
        
    } else if (TB_config.widgets[widgetId].urlsDone == TB_config.widgets[widgetId].ajaxURLs.length) {

        // hide loading message
        TB_hideLoader(widgetId);

        // if nothing added after we are through all sources let user know
        if(jQuery('#' + widgetId + '-mc > ul.tb_tweetlist').children('li').size() == 0) {
...
//console.log('got response');
                ...
                if (json.error) {
                    ...
                    TB_checkComplete(widgetId,json.error);
               }
                else {
...
//console.log('error in ajax: ' + textStatus + ' err: ' + errorThrown);
                TB_config.widgets[widgetId].urlsDone++;
                TB_checkComplete(widgetId,'connection_err');
            }
        });
    });
}
...
//console.log(i + ": " + tweetJson );
        ...
        tweetExists = false;
        jQuery('li.tb_tweet').each(function() {
        ...
        if (TB_config.widgets[widgetId].tweetsShown == 0) {
            ...
            // add at the end
            jQuery('#'+widgetId+'-mc > ul.tb_tweetlist').append(tb_tweet.getHTML());
        ...
        // if tweet is newer than the newest
            ...
            jQuery('#'+widgetId+'-mc > ul.tb_tweetlist').prepend(tb_tweet.getHTML());
            TB_config.widgets[widgetId].tweetsShown++;
        ...
        // if tweet is in the middle
            ...
            nextTweetId;
            jQuery('#'+widgetId+'-mc > ul.tb_tweetlist > li.tb_tweet').each(function(i,nextTweet){
...
function TB_enforceLimit(widgetId) {
    ...
    if (TB_config.widgets[widgetId].tweetsShown == TB_config.widgets[widgetId].tweetsNum) {
        ...
        nextToLastTweet = lastTweet.prev('li.tb_tweet');
...
function TB_showMessage(widgetId, messageId, msg, keepOnScreen){
    ...
    if(!widgetId) {
        jQuery('ul.tb_tweetlist').before('<div id="msg_' + messageId + '" class="tb_msg" style="display:none;">' + msg + '</div>');
        return;
    }
    
    // if it doesn't exist
    if (!jQuery('#' + widgetId + '-mc').children('#msg_' + messageId).length) {
        jQuery('#' + widgetId + '-mc').children('ul.tb_tweetlist').before('<div id="msg_' + messageId + '" class="tb_msg" style="display:none;">' + msg + '</div>');
...
function TB_tweet(tweetJson) {
    ...
    else if (typeof(tweetJson.user) != 'undefined' && typeof(tweetJson.user.screen_name) != 'undefined') {
    ...
    this.selectRelevantSources = function(urlSources) {
        ...
        jQuery.each(urlSources.split(','),function(i,src) {
            if (sourceScreenName == src || ((typeof(jsonCode.text) !== 'undefined') && jsonCode.text.indexOf(src) > 0)){
                allSources.push(src);
            }
        });
    ...
    // if not, use default format
        ...
        template = '<li id="{0}" class="tb_tweet tweet">';
        template += '<a class="tb_photo thumbnail" target="_blank" rel="nofollow" href="https://twitter.com/{1}"><img src="{2}" alt="{1}"></a>';
        template += '<div class="details">';
        template += '<div class="tweet-title tb_author"><span class="name">{7}</span> <a rel="nofollow" target="_blank" href="https://twitter.com/{1}">{1}</a></div>';
        if (!TB_config.general_seo_tweets_googleoff && TB_config.general_seo_footer_googleoff) {
            template += '<!--googleoff: index-->';
        }
        template += '<p class="tweet-message">{3}';
        // show source if requested
        if (TB_config['widget_show_source'] && tweetJson.source) {
            template += ' <br>'  + ' {6}';  // + TB_labels.from
        }
        template += '</p>';
        
        // show timestamp
        template += '<time class="tweet-time" datetime="{5}"><a rel="nofollow" target="_blank" href="http://twitter.com/{1}/statuses/{4}">{5}</a></time>';     
       ...
       if (TB_config.widget_show_follow_link || TB_config.widget_show_reply_link) {
            ...
            if (TB_config.widget_show_reply_link) {
                template += '<a rel="nofollow"  target="_blank" href="http://twitter.com/home?status=@{1}%20&in_reply_to_status_id={4}&in_reply_to={1}">' + TB_labels.reply + '</a>';
            }
            if (TB_config.widget_show_follow_link && TB_config.widget_show_reply_link) {
                template += ' | ';
            }
            if (TB_config.widget_show_follow_link) {
                template += '<a rel="nofollow"  target="_blank" href="http://twitter.com/{1}">' + TB_labels.follow + ' {1}</a>';
            }
            template += '</div>'; 
        }
        // end tweet    
    ...
    this.getHTML = function() {
        ...
        sourceHtml  = '',
        userName    = '';
        if (typeof(this.jsonCode.text) !== 'undefined'){
            textHtml = this.jsonCode.text;
        }
        ...
        if (typeof(this.screenName) === 'undefined'){
            this.screenName = '';
        }
        // link URLs
        textHtml = textHtml.replace(/(https?:\/\/\S+)/gi, '<a rel="nofollow" target="_blank" href="$1">$1</a>');
        // link screen names
        textHtml = textHtml.replace(/\@([\w]+)/gi,'<a rel="nofollow" target="_blank" href="http://twitter.com/$1">@$1</a>'); 
        // link hash tags
        textHtml = textHtml.replace(/\#(\S+)/gi,'<a rel="nofollow" target="_blank" href="http://twitter.com/search?q=%23$1">#$1</a>'); 

        if (tweetJson.profile_image_url) {
            imageUrl = tweetJson.profile_image_url;
        }
        else if ((typeof(tweetJson.user) !== 'undefined') && (typeof(tweetJson.user.profile_image_url) !== 'undefined')) {
        ...
        if (typeof(tweetJson.source) !== 'undefined') {
        ...
        if ((typeof(this.jsonCode.user) !== 'undefined') && (typeof(this.jsonCode.user.name) !== 'undefined')) {
            userName = this.jsonCode.user.name;
        }

        // return formatted string
        return template.format(this.divId,'@' + nameHtml,imageUrl,textHtml,tweetJson.id_str,dateHtml,sourceHtml,userName);
    }
...
// initialize
//TB_addLoadEvent(TB_start); 
jQuery(document).ready(function () {
    // Accordion events 
    if (jQuery('section.accordion').length > 0) {
        jQuery('section.accordion').on('shown.bs.collapse', function (e) {
            TB_start();
        });
    }
});
//jQuery(window).bind("load", TB_start);
