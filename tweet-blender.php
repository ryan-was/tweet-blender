<?php
/*
Plugin Name: Tweet Blender [Modified]
...
function tb_db_install() {
    ...
    $sql = "CREATE TABLE " . $table_name . " (
        ...
        PRIMARY KEY (`ID`),
        UNIQUE KEY `div_id` (`div_id`,`source`),
        KEY `source` (`source`)
...
function tb_load_js() {
    ...
    if (isset($tb_o['general_timestamp_format']) && ($tb_o['general_timestamp_format'] != '')) {
        wp_enqueue_script('phpdate', '/' . PLUGINDIR . '/tweet-blender/js/jquery.phpdate.js', array('jquery'), '2.0.0', true);
    ...
    // load main JS code
    wp_enqueue_script('tb-main', '/' . PLUGINDIR . '/tweet-blender/js/main.js', $dependencies, '2.0.0', true);
...
/**
 * WAS : updated this function to don't show tweets on the page avoiding caching. Let the JS upload on page 
 * @param string $mode
 * @param unknown $instance
 * @param unknown $widget_id
 * @param unknown $tb_o
 * @param string $noLoad // created by WAS
 * @return string
 */
function tb_create_markup($mode = 'widget',$instance,$widget_id,$tb_o,$noLoad=false) {
    $html = '';
    $html .= tb_create_markup_header($widget_id);

    if ($mode == 'chart') {
        $html .= '<div class="tb_tweetchart" id="' . $widget_id . '-chart"></div>';
        $html .= ($noLoad?'':tb_get_cached_tweets_html($mode,$instance,$widget_id));
    }
    else {
        if (isset($tb_o['general_seo_tweets_googleoff']) && $tb_o['general_seo_tweets_googleoff']) {
            $html .= '<!--googleoff: index--><ul class="tb_tweetlist list-conversation">' . ($noLoad?'':tb_get_cached_tweets_html($mode,$instance,$widget_id)) . '</ul><!--googleon: index-->';
        }
        else {
            $html .= '<ul class="tb_tweetlist list-conversation">' . ($noLoad?'':tb_get_cached_tweets_html($mode,$instance,$widget_id)) . '</ul>';
        }
    }
    return $html;
}
...
function tb_create_markup_header($widget_id) {
    ...
    $html .= '<a class="tb_refreshlink" href="javascript:TB_blend(\'' . $widget_id . '\');" title="Refresh Tweets"><img src="' . plugins_url('tweet-blender/img/ajax-refresh-icon.gif') . '" alt="Refresh" /></a>';
    $html .= '</div>';
    $html .= '</div>';
    return $html;    
}
