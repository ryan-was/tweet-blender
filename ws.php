...
require_once("/var/www/live/public_html/wp-config.php");
$includesDir = defined('ABSPATH')?ABSPATH.WPINC:'/var/www/live/public_html/wp-includes';
...
if (!isset($wp_json) || !is_a($wp_json, 'Services_JSON') ) {
    if (file_exists($includesDir . '/class-json.php')) {
        require_once( $includesDir . '/class-json.php' );
    }
    else {
...
if (in_array($_GET['action'],array('search','list_timeline','user_timeline','favorites'))) {
    ...
    if ($_GET['action'] == 'search') {
        ...
        if (isset($params['from'])) {
            $sources = explode(' OR ',$params['from']);
            // add the @ sign
        ...
        elseif (isset($_GET['ors'])) {
            ...
            $tmp_sources = explode(' OR ',$params['q']);
        ...
        if (!empty($sources)) {
            foreach ($sources as $k => $val) {
                if (($val[0] == '"') && ($val[strlen($val) - 1] == '"')) {
                    $sources[$k] = substr($val,1,strlen($val) - 2);
                }
            } 
        }
...
//echo('endpoint=' . $api_endpoint . ' optimal delay=' . $optimal_seconds_between_requests . ' sec since last request=' . $seconds_since_last_request . ' limit remaining=' . $tb_o['rate_limit_data'][$api_endpoint]['remaining']);
...
//error_log('**** json data: ' . $json_data);
        ...
        // else, try to get it from cache and if that fails report an error
            ...
            $json_data = tb_get_cached_tweets_json($sources);
            if (!empty($json_data) && ($json_data !== '[]')) {
                echo $json_data;
            }
            else {
                echo $wp_json->encode(array('error' => 'no_tweets_err', 'msg' => __('No cache. Connection status code', 'tweetblender') . ' ' . (isset($oAuth->http_code)?$oAuth->http_code:'')));
            }
            exit;
        }
    }
    // if we've reached the limit, just get data from cache
        ...
        if (!empty($json_data) && ($json_data !== '[]')) {
            ...
            echo $wp_json->encode(array('error' => 'no_tweets_err', 'msg'  => __('Reached Twitter API limit and there is no cache.', 'tweetblender')));
        }
        exit;
    }
    ...
    echo $wp_json->encode(array('error' => 'no_tweets_err', 'msg' => __('Do not know what you want.', 'tweetblender'))); 
}
