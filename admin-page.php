...
function tb_admin_load_scripts() {
    ...
    wp_enqueue_script('tb-admin', '/' . PLUGINDIR . '/tweet-blender/js/admin.js',array('jquery','jquery-ui-core','jquery-ui-tabs','jquery-fancybox','tb-lib'),false,true);
}
...
function tb_admin_page() {
	...
	if (isset($_GET['install_addon'])) {
		...
		if (isset($_REQUEST['txn_id']) && $item_number) {
			tb_save_txn_id($item_number, $_REQUEST['txn_id']);
		}
...
<script type="text/javascript">
	...
	TB_ajax_nonce  = '<?php echo wp_create_nonce('tb_cache_manager'); ?>';
</script>
