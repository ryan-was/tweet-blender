...
class TweetBlender extends WP_Widget {
    ...
    private $maxTime =  3600; // 60min old
    
    // constructor     
    function TweetBlender() {
        parent::WP_Widget('tweetblender', __('Tweet Blender', 'tweetb'), array('description' => __('Shows tweets for @users, @user/lists, #hashtags, and keywords blended together', 'tweetblender')));    
    }
    ...
    function widget($args, $instance) {
        ...
        if ($post == null || ($post->ID != $archive_post_id && $archive_page_url != "http://" . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'])) {
            ...
            $noLoadTweets = false;
            if ($instance['widget_refresh_rate'] == 1) {
                $lastUpdated = tb_last_time_cached_tweets($instance,$sources);
                $last = (!empty($lastUpdated) && isset($lastUpdated[0]->last_date))?$lastUpdated[0]->last_date:0;
                $instance['widget_refresh_rate'] = ((strtotime('now') - strtotime($last)) > $this->maxTime)?$instance['widget_refresh_rate']:0;
                $noLoadTweets = (strtotime('now') - strtotime($last)) > $this->maxTime;
            }
            
            // add configuration options
            echo '<form id="' . $this->id . '-f" class="tb-widget-configuration" action="#"><div>';
            echo '<input type="hidden" name="sources" value="' . esc_attr(implode(',',$sources)) . '" />';
            ...
            echo '<div id="'. $this->id . '-mc" class="conversation-box">';
            echo tb_create_markup('widget',$instance,$this->id,$tb_o,$noLoadTweets);
        ...
        if (!$tb_o['oauth_access_token']) {                
            ...
            if (isset($tb_o['widget_check_sources']) && $tb_o['widget_check_sources']) {
                foreach($new_sources as $src) {
                    ...
                    if ($src != '') {
                        list($is_ok,$msg,$log) = $this->check_source($source_to_check,$tb_o);
    ...
    function check_source($src,$tb_o) {
        ...
        if (stripos($src,'@') === 0 && stripos($src,'/') > 1) {
            $api_url = 'https://api.twitter.com/1.1/lists/statuses.json';
            $api_params = array(
                'owner_screen_name' => substr($src,1,strpos($src,'/')),
                'slug' => substr($src,strpos($src,'/'))
            );
        }
        ...
        // else assume it's a hashtag or keyword
            ...
            $api_params = array('q' => urlencode($src));
        }
...
add_action( 'widgets_init', create_function('', 'return register_widget("TweetBlender");') );
